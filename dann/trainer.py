...
class Trainer:
    def __init__(
        ...
        # ]
        params_to_update = [
            {"params": self.model.feature_backbone.parameters(), "lr": config['optimizer']['lr']},
            {"params": self.model.domain_classifier.parameters(), "lr": config['optimizer_D']['lr']}
        ]
        self.optimizer = AdamW(params_to_update, **config['optimizer'])
    ...
    def train(self, source_loader, target_loader):
        ...
        for _ in range(max(len(source_loader), len(target_loader))):
            ...
            # # pseudo labeling
            # pseudo_labels = torch.argmax(target_semantic_outputs['out'], dim=1).long()  # get the predicted class labels

            # prediction_probs = F.softmax(target_semantic_outputs['out'], dim=1)  # get the softmax probabilities
    ...
    def evaluate(self, loader):
        ...
        for images, masks in loader:
            ...
            _, predictions = torch.max(semantic_outputs['out'], 1) 
